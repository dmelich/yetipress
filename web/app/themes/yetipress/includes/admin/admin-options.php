...
add_action( 'genesis_theme_settings_metaboxes', 'imp_remove_theme_settings_metaboxes' );
/**
 ...
 * @since 2.0.0
 */
function imp_remove_theme_settings_metaboxes( $_genesis_theme_settings_pagehook ) {
...
add_filter( 'genesis_theme_settings_defaults', 'imp_theme_settings_defaults' );
/**
 * Set default values for custom theme options.
 *
 * @since 2.0.0
 */
function imp_theme_settings_defaults( $defaults ) {

	$defaults['imp_production_on']         = false;
	$defaults['imp_assets_version']        = null;
...
add_action( 'genesis_settings_sanitizer_init', 'imp_settings_sanitizer' );
/**
 * Set filters for custom theme options.
 *
 * @since 2.0.0
 */
function imp_settings_sanitizer() {
...
add_action( 'genesis_theme_settings_metaboxes', 'imp_theme_settings_metaboxes' );
/**
 * Add meta boxes for custom theme options.
 *
 * @since 2.0.0
 */
function imp_theme_settings_metaboxes( $pagehook ) {

	add_meta_box(
		'bfg-environment-settings',
		__( 'Environment', CHILD_THEME_TEXT_DOMAIN ),
		'imp_environment_settings_box',
		$pagehook,
		'main',
		'high'
	);
 ...
 * @since 2.0.0
 */
function imp_environment_settings_box() {
	...
	<table class="form-table">
		<tbody>
			<tr valign="top">
				...
				<td>
					<p>
						<label for="<?php echo GENESIS_SETTINGS_FIELD; ?>[imp_production_on]">
							<input type="checkbox" name="<?php echo GENESIS_SETTINGS_FIELD; ?>[imp_production_on]" id="<?php echo GENESIS_SETTINGS_FIELD; ?>[imp_production_on]" value="1" <?php checked( genesis_get_option('imp_production_on'), 1 ); ?>>
			...
			<tr valign="top">
				<th scope="row">
					<label for="<?php echo GENESIS_SETTINGS_FIELD; ?>[imp_assets_version]">
						<?php _e( 'Assets Version Number:', CHILD_THEME_TEXT_DOMAIN ); ?>
					</label>
				</th>
				<td>
					<p>
						<input type="text" name="<?php echo GENESIS_SETTINGS_FIELD; ?>[imp_assets_version]" value="<?php echo esc_attr( genesis_get_option('imp_assets_version') ); ?>" class="regular-text" id="<?php echo GENESIS_SETTINGS_FIELD; ?>[imp_assets_version]">
					</p>
